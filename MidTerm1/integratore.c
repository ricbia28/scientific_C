...
void rk4(param s){
  ...
  for(i=0;i<s.n;i++){
    printf("%lg %.14lf %.14lf\n",i*s.dt ,x ,v);
    dx1=v*s.dt;
    dv1=ac(x,v,i*s.dt)*s.dt;
    
    dx2=(v+dv1/2)*s.dt;
    dv2=ac(x+dx1/2,v+dv1/2,i*s.dt+s.dt/2)*s.dt;
    
    dx3=(v+dv2/2)*s.dt;
    dv3=ac(x+dx2/2,v+dv2/2,i*s.dt+s.dt/2)*s.dt;
    
    dx4=(v+dv3)*s.dt;
    dv4=ac(x+dx3,v+dv3,i*s.dt+s.dt)*s.dt;
...
int main(int argc, char *argv[]){
    ...
    if(argc != 6){
        ...
        fprintf(stderr,"usage:%s 1)velocita' 2)dt 3)tmax 4)algoritmo 5)coefficente\n\n",argv[0]);
	...
	fprintf(stderr,"Sono disponibili 7 tipi diversi di algoritmo:\n");
	fprintf(stderr,"1:Eulero, 2:Eulero-Cromer, 3:LeapFrog, 4:Verlet-Velocita', 5:Verlet-Posizioni,\n");
	fprintf(stderr,"6:Runge-Kutta del secondo ordine, 7:Runge-Kutta del quarto ordine\n\n");
