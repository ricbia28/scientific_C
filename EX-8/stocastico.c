...
param input(int argc, char *argv[]){
    ...
    if (argc > 6 || argc < 4){
        ...
        fprintf(stderr,"usage:%s 1)numero di traiettorie generate 2)tempo massimo di integrazione 3)modalita' 4)base della potenza 5)larghezza del bin\n\n",argv[0]);
        ...
        fprintf(stderr,"Il parametro 'base della potenza' serve per le modalità 2 e 3 e sarebbe la base delle potenze da prendere come istanti,\n"); 
        fprintf(stderr,"Se si usa la modalità 1 può essere omesso, se non viene inserito verrà posto uguale a 2 \n\n"); 
        fprintf(stderr,"Il parametro 'larghezza del bin' serve per la modalità 3 e sarebbe la larghezza dei singoli bin dell'istogramma generato,\n"); 
        fprintf(stderr,"Se si usa la modalità 1 o 2 può essere omesso, se non viene inserito verrà posto uguale a 2 \n\n"); 
        fprintf(stderr,"Se si usa la modalità 3 il parametro 'base della potenza' deve essere necessariamente inserito,\n"); 
        fprintf(stderr,"mentre se il parametro 'larghezza bin' viene omesso verrà posto automaticamente pari a 2\n\n"); 
    ...
    if(argv[5]==NULL) rb.l = 2;
    else rb.l = atoi(argv[5]);
...
void istogrammi(param s){
    ...
    for(i=0;i<s.imax;i++){
        ...
        h[i] = (int*)calloc(2*(int)(meastime/s.l)+1,sizeof(int)); 
        if(h[i]==NULL){
        fprintf(stderr,"Errore nell'allocazione");
        exit(EXIT_FAILURE);
        }
    ...
    printf("#Numero traiettorie utilizzate-> %i \n#Tempo massimo-> %i\n#Potenza utilizzata-> %i\n#Larghezza del bin-> %i\n",s.ntra, s.tmax, s.b, s.l);

    for(i=1;i<s.imax;i++){ 
        for(j=0;j<=(2*(int)(meastime/s.l));j++){
            printf("%i %i %lg\n",meastime, s.l*(j-(meastime/s.l)), (double)(h[i][j])/(s.ntra*s.l));
        }
        printf("\n\n");
        meastime*=s.b;
...
int main(int argc, char *argv[]){
    ...
    if(rb.scelta==1)traiettorie(rb);

    if(rb.scelta==2)statistica(rb);
